...
# This script will build databroker-perf for different architectures uing cross
...
# You run it like
#
# ./build-databroker-perf.sh plattforms
#
# where platform can be one or more of
#
# arm64, amd64, riscv64, i.e. the following are valid commandlines
#
# ./build-databroker-perf.sh amd64
# ./build-databroker-perf.sh amd64 arm64 riscv64
#
...
# KUKSA_DATABROKERPERF_SBOM to "y(es)" or "true",
#
...
# if [ -z "$KUKSA_DATABROKERPERF_FEATURES" ]; then
#     # If not set, assign a default value
#     KUKSA_DATABROKERPERF_FEATURES="databroker-perf/default"
# fi

SBOM=0
# Check whether to build SBOM
if [ ! -z "$KUKSA_DATABROKERPERF_SBOM" ]; then
    # If set, check whether it is "y"
    if [[ $KUKSA_DATABROKERPERF_SBOM =~ ^[Yy](es)?$ || $KUKSA_DATABROKER_SBOM =~ ^[Tt](rue)?$ ]]; then
        SBOM=1
    fi
fi
...
# echo  "Building with features: $KUKSA_DATABROKERPERF_FEATURES"
...
function build_target() {
    ...
    echo "Building databroker-perf for target $target_rust"
    # cross build --target $target_rust --target-dir ./target-$target_docker --features $KUKSA_DATABROKERPERF_FEATURES --bin databroker-perf --release
    cross build --target $target_rust --target-dir ./target-$target_docker --bin databroker-perf --release

    echo "Prepare $target_docker dist folder"
    rm -rf ./dist/$target_docker || true
    mkdir ./dist/$target_docker
    cp ./target-$target_docker/$target_rust/release/databroker-perf ./dist/$target_docker
    ...
    if [[ $SBOM -eq 1 ]]; then
        ...
        cp ./databroker-perf_bin.cdx.json ./dist/$target_docker/sbom.json
        rm -rf ./dist/$target_docker/thirdparty-licenses || true
        collectlicensefiles ./databroker-perf_bin.cdx.json ./dist/$target_docker/thirdparty-licenses --curation ./scripts/licensecuration.yaml
    fi
}
